...
ldnmp_web_status() {
	...
	while true; do
		...
		echo "サイトディレクトリ"
		echo "------------------------"
		echo -e "データ${gl_hui}/home/web/html${gl_bai}証明書${gl_hui}/home/web/certs${gl_bai}     配置 ${gl_hui}/home/web/conf.d${gl_bai}"
		echo "------------------------"
		echo ""
		echo "操作"
		echo "------------------------"
...
dd_xitong() {
		...
		dd_xitong_4() {
		  echo -e "重装后初始用户名: ${gl_huang}Administrator${gl_bai}初期パスワード:${gl_huang}123@@@${gl_bai}初期ポート:${gl_huang}3389${gl_bai}"
		  echo -e "続行するには任意のキーを押してください..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
	...
	backup_docker() {
		...
		for c in "${TARGET_CONTAINERS[@]}"; do
			...
			if is_compose_container "$c"; then
				echo -e "${gl_kjlan}检测到 $cdocker-compose コンテナーです${gl_bai}"
...
for name, provider in list(providers.items()):
    ...
    if api not in SUPPORTED_APIS:
        summary.append(f'ℹ️ {name} をスキップ: 直接の /models 検証はサポートされていません (api={api})')
        continue
...
PY
	...
	# OpenClaw API プロトコル検出 (優先応答 -> チャット/完了 -> 完了)
	...
	while IFS=$'\t' read -r rec_type idx name base_url model_count api_type latency_txt latency_level; do
		case "$rec_type" in
			...
			ROW)
				...
				printf '%b\n' "[$idx] ${name} | API: ${base_url}|合意：${api_type} | 模型数量: ${gl_huang}${model_count}${gl_bai}|遅延/ステータス:${latency_color}${latency_txt}${gl_bai}"
				;;
		esac
...
if api not in SUPPORTED_APIS:
    print(f'❌ プロバイダー {target} の現在の api={api} は、直接 /models 同期をサポートしていません')
    raise SystemExit(3)
...
PY
		...
		echo "openclaw メモリ インデックス --force をすぐに実行することをお勧めします。"
		read -e -p "openclaw メモリ インデックス --force をすぐに実行しますか? (はい/いいえ):" rebuild_choice
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}63.  ${color63}OpenWebUI自托管AI平台 ${gl_huang}★${gl_bai}             ${gl_kjlan}64.  ${color64}ITツールツールボックス"
