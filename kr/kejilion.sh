	...
	read -e -p "请输入你的选择: " choice
	 case $choice in
		1)
			...
			while true; do
				read -e -p "애플리케이션 외부 서비스 포트를 입력하고 Enter 키를 누르면 기본적으로 사용됩니다.${docker_port}端口: " app_port
...
format_partition() {
	...
	# 格式化分区
	echo "正在格式化分区 /dev/$PARTITION~을 위한$FS_TYPE ..."
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  b)
	  	...
	  	while true; do
			...
			case "$choice" in
			  [Yy])
				kj_ssh_read_host_port "원격 서버 IP를 입력하세요:" "目标服务器SSH端口 [默认22]: " "22"
				local remote_ip="$KJ_SSH_HOST"
				local TARGET_PORT="$KJ_SSH_PORT"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}39.  ${gl_bai}명령 즐겨찾기${gl_huang}★${gl_bai}                       ${gl_kjlan}40.  ${gl_bai}网卡管理工具"
	  echo -e "${gl_kjlan}------------------------"
