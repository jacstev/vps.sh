...
EOF
	...
	add-openclaw-provider-interactive() {
		...
		if [[ $? -eq 0 ]]; then
			...
			echo "✅ 프로토콜은 다음과 같이 자동으로 식별되었습니다.$DETECTED_API"
		fi
...
EOF
	...
	openclaw_read_import_path() {
		...
		echo "팁: 파일 이름을 입력하면 기본적으로 백업 디렉터리에서 검색됩니다. /가 포함된 경로를 입력하면 전체 경로와 비교하여 확인됩니다." >&2
...
PY
	...
	openclaw_memory_fix_index() {
		...
		echo "적용 가능한 시나리오: 인덱스 분자 > 분모(중복 컬렉션으로 인해 비정상적인 계산이 발생함)"
		...
		while true; do
			...
			case "$scheme_choice" in
				1)
					...
					read -e -p "이 제도를 적용해야 할까요? (예/아니요):" apply_choice
