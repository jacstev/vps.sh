...
dd_xitong() {
		...
		dd_xitong_3() {
		  echo -e "재설치 후 초기 사용자 이름:${gl_huang}root${gl_bai}  初始密码: ${gl_huang}123@@@${gl_bai}초기 포트:${gl_huang}22${gl_bai}"
		  echo -e "계속하려면 아무 키나 누르세요..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
...
add_connection() {
	...
	kj_ssh_read_host_user_port "IP 주소를 입력하세요:" "请输入用户名 (默认: root): " "포트 번호를 입력하십시오(기본값: 22):" "root" "22"
...
for name, provider in list(providers.items()):
    ...
    if api not in SUPPORTED_APIS:
        summary.append(f'ℹ️ {name} 건너뛰기: 직접 /models 유효성 검사가 지원되지 않습니다(api={api})')
        continue
...
PY
	...
	# OpenClaw API 프로토콜 감지(우선순위 응답 -> 채팅/완료 -> 완료)
...
if api not in SUPPORTED_APIS:
    print(f'❌ 공급자 {대상} 현재 api={api}, 직접 /models 동기화를 지원하지 않습니다.')
    raise SystemExit(3)
...
PY
		...
		echo "즉시 실행하는 것이 좋습니다: openclaw memory index --force"
		read -e -p "openclaw memory index --force를 즉시 실행하시겠습니까? (예/아니요):" rebuild_choice
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "设置${docker_name}로그인 키(sk-다음과 같이 시작하는 문자와 숫자의 조합): sk-159kejilionyyds163:" app_passwd
